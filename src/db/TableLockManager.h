...
public:
  ...
  [[nodiscard]] std::shared_lock<std::shared_mutex> acquireRead(const std::string& table_name)
  {
    return std::shared_lock<std::shared_mutex>(getOrCreateLock(table_name));
  }

  [[nodiscard]] std::unique_lock<std::shared_mutex> acquireWrite(const std::string& table_name)
  {
