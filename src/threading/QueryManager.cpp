...
{
  return std::string(exc.what()).find("WaitQuery completed") != std::string::npos;
}
...
void QueryManager::executeQueryForTable(QueryManager* manager, const std::string& table_name)
{
  ...
  {
    std::counting_semaphore<>* sem_ptr = manager->getSemaphoreForTable(table_name);
    if (sem_ptr == nullptr)
    {
...
std::counting_semaphore<>* QueryManager::getSemaphoreForTable(const std::string& table_name)
{
  const std::scoped_lock lock(table_map_mutex);
...
std::optional<QueryManager::QueryEntry> QueryManager::dequeueQuery(const std::string& table_name)
{
  const std::scoped_lock lock(table_map_mutex);
