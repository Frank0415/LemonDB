...
# Find source files tracked by git
SRC_FILES=$(git -C "${REPO_ROOT}" ls-files '*.c' '*.cc' '*.cxx' '*.cpp' '*.c++' '*.h' '*.hh' '*.hpp' '*.hxx' || true)
TUS=$(git -C "${REPO_ROOT}" ls-files '*.c' '*.cc' '*.cxx' '*.cpp' '*.c++' || true)
...
if tool_exists python3 && python3 -m pip show cpplint >/dev/null 2>&1; then
  if [[ -n "${SRC_FILES}" ]]; then
    ...
    git -C "${REPO_ROOT}" ls-files '*.c' '*.cc' '*.cxx' '*.cpp' '*.c++' '*.h' '*.hh' '*.hpp' '*.hxx' -z | \
...
if tool_exists clang-tidy; then
  if [[ -f "${BUILD_DIR}/compile_commands.json" && -n "${TUS}" ]]; then
    ...
    done < <(git -C "${REPO_ROOT}" ls-files '*.c' '*.cc' '*.cxx' '*.cpp' '*.c++' -z || true)
  else
