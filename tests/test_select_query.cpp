...
TEST_F(QuerySelectSwapTest, Swap_ClassAndStudentID_WithWhere_MatchesSpec) {
  SwapQuery q("Student", {"class","studentID"}, { C("class","<","2015") });
  auto res = q.execute();
  ASSERT_TRUE(res->success());
  const std::string out = asString(res);
  // Two rows should be affected: Bill_Gates, Steve_Jobs
...
TEST_F(QuerySelectSwapTest, Swap_NoMatch_AffectedZero) {
...
TEST_F(QuerySelectSwapTest, Swap_SameField_NoEffect) {
...
TEST_F(QuerySelectSwapTest, Swap_FullTable_ThenStateMatchesExpectations) {
  ...
  const auto& stu = Database::getInstance()["Student"];
  // After a full-table swap from the seeded state:
  // Bill_Gates -> studentID=2014, class=400812312
  // Jack_Ma    -> studentID=2015, class=400882382
  // Steve_Jobs -> studentID=2014, class=400851751
  EXPECT_EQ(geti(stu,"Bill_Gates","studentID"), 2014);
  EXPECT_EQ(geti(stu,"Bill_Gates","class"), 400812312);
